...
//			redirect('login');	 
	...
	{ 
		...
		$img = $this->myupload->do_upload(
	    	...
	    	'./application/modules/recruitment/assets/images/','resume'
                );
...
public function workexperience_view()
 ...
 for ($i=0; $i < sizeof($comname); $i++) {
    ...
    public function candidate_accept() {
        $data['name'] = $_GET['name'];
        $data['position'] = "Lead Programmer";
        $data['module'] = "recruitment";
        $data['employee_id'] = $_GET['id'];
        $data['page'] = "offer_letter";
        echo Modules::run('template/layout', $data); 
    }
